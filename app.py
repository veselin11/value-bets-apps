...
# Логика за извличане на стойностни залози (над 5% value)
        ...
        for bookmaker in match.get("bookmakers", []):
            for market in bookmaker.get("markets", []):
                ...
                for outcome in market.get("outcomes", []):
                    ...
                    implied_prob = 1 / odds
                    fair_prob = 0.30  # Примерна оценка на вероятност (30%)
...
# Основен поток
with st.spinner("Зареждане на реални залози..."):
    data, error = get_odds()
    if error:
        st.error(error)
    elif data:
        ...
        if not df.empty:
            st.success(f"Открити {len(df)} стойностни залози!")
            st.dataframe(df.sort_values(by="Value %", ascending=False), use_container_width=True)
        else:
            st.warning("Няма намерени стойностни залози в момента.")
    else:

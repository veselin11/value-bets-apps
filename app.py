...
def main():
    ...
    def process_team_stats(matches, team_type):
        if not matches:
            return {"avg_goals": 1.0, "win_rate": 0.5}

        if team_type == "home":
            goals = [m["score"]["fullTime"]["home"] for m in matches]
            wins = sum(1 for m in matches if m["score"]["fullTime"]["home"] > m["score"]["fullTime"]["away"])
        else:
            goals = [m["score"]["fullTime"]["away"] for m in matches]
            wins = sum(1 for m in matches if m["score"]["fullTime"]["away"] > m["score"]["fullTime"]["home"])
    ...
    home_stats = process_team_stats(home_stats_raw, "home")
    away_stats = process_team_stats(away_stats_raw, "away")

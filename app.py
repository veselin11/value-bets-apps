...
# Данни – само нашите реални прогнози
def get_real_predictions():
    return [
        {"Дата": str(date.today() - timedelta(days=1)), "Мач": "Elfsborg - Molde", "Прогноза": "ГГ", "Коеф": 1.85, "Сума": 20, "Резултат": "✅ Печеливш",
         "Обосновка": "И двата отбора показват висока резултатност, а Molde бележи средно над 1.5 гола като гост.", "Източник": "наш"},
        {"Дата": str(date.today() - timedelta(days=1)), "Мач": "AIK - Kalmar", "Прогноза": "1", "Коеф": 1.75, "Сума": 30, "Резултат": "✅ Печеливш",
         "Обосновка": "AIK е силен домакин и се намира във възход, докато Kalmar е в слаба форма и с кадрови проблеми.", "Източник": "наш"},
        {"Дата": str(date.today() - timedelta(days=1)), "Мач": "Avai - Coritiba", "Прогноза": "1", "Коеф": 2.00, "Сума": 20, "Резултат": "❌ Губещ",
         "Обосновка": "Avai запазва стабилност у дома и има добър баланс срещу Coritiba.", "Източник": "наш"},

        {"Дата": str(date.today()), "Мач": "Kairat - Olimpija", "Прогноза": "Под 2.5", "Коеф": 1.65, "Сума": 40, "Резултат": "Очаква се",
         "Обосновка": "И двата отбора играят дефанзивно в евротурнирите, очаква се предпазлив подход.", "Източник": "наш"},
        {"Дата": str(date.today()), "Мач": "Malmo - Saburtalo", "Прогноза": "Над 2.5", "Коеф": 1.60, "Сума": 20, "Резултат": "Очаква се",
         "Обосновка": "Malmo играе офанзивно у дома, а Saburtalo допуска голове почти във всеки мач.", "Източник": "наш"},
        {"Дата": str(date.today()), "Мач": "Uruguay W - Argentina W", "Прогноза": "1", "Коеф": 2.00, "Сума": 10, "Резултат": "Очаква се",
         "Обосновка": "Уругвай е в по-добра форма и има психологическо предимство след предишни победи.", "Източник": "наш"},
    ]
...
# Зареждаме само НАШИТЕ прогнози (по източник)
df_all = st.session_state.df.copy()
df_our = df_all[df_all["Източник"] == "наш"]

# Таблица без обосновка
df_table = df_our.drop(columns=["Обосновка", "Източник"])

# Заглавие и банка
st.title("⚽ Нашите прогнози")

st.session_state.bank = calculate_bank(df_our, st.session_state.initial_bank)
st.subheader("💰 Банка")
st.metric("Текущ баланс", f"{st.session_state.bank:.2f} лв")

# Таблица
st.subheader("📋 Таблица с прогнози")
df_styled = df_table.style.apply(highlight_result, axis=1)
st.dataframe(df_styled, use_container_width=True)

# Обосновка
st.subheader("🔎 Виж обосновка по мач")

match_options = [f"{row['Дата']} | {row['Мач']}" for _, row in df_our.iterrows()]
selected = st.selectbox("Избери мач", match_options)

selected_row = df_our.iloc[match_options.index(selected)]

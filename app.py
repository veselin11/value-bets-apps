...
sports_url = f"https://api.the-odds-api.com/v4/sports/?apiKey={ODDS_API_KEY}" sports_response = requests.get(sports_url) sports = sports_response.json()

print("Автоматичен детектор на стойностни футболни залози с форма и H2H\n")

for sport in sports: sport_key = sport['key'] sport_title = sport['title'] print(f"Лига: {sport_title}")
